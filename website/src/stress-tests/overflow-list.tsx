...
type OverflowListInstanceCount = (typeof OVERFLOW_LIST_INSTANCE_COUNTS)[number];
...
type OverflowListStressListProps = {
  actionCount: number;
  detailed: boolean;
...
const InstanceCountSelect = (props: InstanceCountSelectProps) => {
  ...
  const handleValueChange = (nextValue: string) => {
    ...
    if (OVERFLOW_LIST_INSTANCE_COUNTS.some((count) => count === nextCount)) {
  ...
  return (
    <div className="mr-2 flex items-center gap-2">
      <label
        htmlFor="overflow-list-instance-count"
      ...
      <Select value={String(value)} onValueChange={handleValueChange}>
        <SelectTrigger
          id="overflow-list-instance-count"
          size="sm"
          className="w-24"
        >
          <SelectValue />
...
const OverflowListStressList = memo((props: OverflowListStressListProps) => {
    ...
    <div className="grid grid-cols-1 gap-px bg-border sm:grid-cols-2 xl:grid-cols-4">
      {stressInstances.slice(0, instanceCount).map((index) => (
        <div key={index} className="min-w-0 bg-background px-3 py-2">
          ...
          <OverflowList className="flex min-w-0 items-center gap-1">
...
OverflowListStressList.displayName = 'OverflowListStressList';

export const OverflowListStressTest = () => {
